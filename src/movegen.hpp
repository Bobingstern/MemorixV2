...
// Mask Vertical on demand
uint64_t maskVerticalOND(int i){
    return (fileHBB << (i & 7)) ^ BB(i);
}

uint64_t maskDiagonalOND(int i){
    return mask_shift<0x8040201008040201ULL>((i & 7) - (i >> 3)) ^ BB(i);
}
uint64_t maskAntidiagonalOND(int i){
    return mask_shift<0x0102040810204080ull>(7 - (i & 7) - (i >> 3)) ^ BB(i);
}
...
static uint64_t horizontal_atkL(uint64_t notOcc, uint64_t sqMsk) {
...
static uint64_t horizontal_atkR(uint64_t notOcc, uint64_t sqMsk) {
...
uint64_t vertical_attack(uint64_t occ, uint32_t sq) {
    return attack(occ, sq, maskVerticalOND(sq));
}

uint64_t diagonal_attack(uint64_t occ, uint32_t sq) {
    return attack(occ, sq, maskDiagonalOND(sq));
}

uint64_t antidiagonal_attack(uint64_t occ, uint32_t sq) {
    return attack(occ, sq, maskAntidiagonalOND(sq));
}
